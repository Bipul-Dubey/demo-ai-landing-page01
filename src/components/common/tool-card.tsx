...
export const ToolCards: React.FC<IntegrationToolsProps> = ({ items }) => {
  return (
    <section className="w-full py-12 px-4">
      <motion.div
        className="grid grid-cols-1 place-items-center sm:place-items-start sm:grid-cols-2 lg:grid-cols-4 gap-8 max-w-6xl mx-auto"
        variants={container}
        initial="hidden"
        whileInView="show"
      ...
      >
        {items.map((item, i) => (
          <motion.div key={`item-${i}`} variants={cardItem}>
            <ToolCard item={item} key={i} />
          </motion.div>
        ))}
      </motion.div>
    </section>
...
export const ToolCard = ({
  ...
  return (
    <div
      className={cn(
        "flex flex-col items-center sm:items-start text-center sm:text-left space-y-4",
        className
      )}
    >
